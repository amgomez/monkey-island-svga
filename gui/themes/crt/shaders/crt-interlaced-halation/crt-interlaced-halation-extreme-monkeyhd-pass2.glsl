/*
    CRT-interlaced-halation shader - Monkey HD pass2 (extreme)

    Very strong version of the stable Monkey HD CRT preset.
*/
#ifdef GL_ES
precision highp float;
            ...
            {
            #ifdef USEGAUSSIAN
                    vec4 wid = 0.20 + 0.10 * pow(color, vec4(3.0));
                    vec4 weights = vec4(distance / wid);
                    return 0.54 * exp(-weights * weights) / wid;
            #else
...
#ifdef MULTIPASS
                    mul_res += pow(COMPAT_TEXTURE(Source, xy2).rgb, vec3(monitorgamma)) * 0.18;
#endif
                    mul_res *= vec3(cval);
